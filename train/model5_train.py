...
def model5_train(save_to_disk: bool, RedFeatures, RegTargets):
    # Building the model
    model5 = Sequential()
    model5.add(Dense(240, input_shape = (200,), activation='relu'))
    model5.add(Dense(220, activation='relu'))
    ...
    model5.add(Dense(37, activation='relu')) # 41 numerical outputs, but only 37 if we exclude release.
...
# RF, RT = model5_prep()
# model5_train(save_to_disk = True, RF, RT)