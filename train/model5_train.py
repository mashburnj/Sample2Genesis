...
def model5_prep(use_wav: bool):
    ...
    ValidationTargets = pd.read_csv('ValRegisters5.csv', index_col = 0)
    ValidationFeatures = pd.read_csv('ValSpectra5.csv', index_col = 0)
    
    if use_wav:
        SampleWav = pd.read_csv('SampleWav5.csv', index_col = 0)
        SampleFeatures = pd.concat([SampleFeatures, SampleWav], axis = 1, join = 'inner')
        del SampleWav
        ValWav = pd.read_csv('ValWav5.csv', index_col = 0)
...
def model5_train(save_to_disk: bool, TrainFeatures, TrainTargets, ValFeatures, ValTargets):
    # Building the model
    model5 = Sequential([
        layers.Input(shape = (81141,)),
        layers.Reshape((129, 629, 1)),
        layers.Resizing(height = 129, width = 129),
        layers.Normalization(mean = 126, variance = 165000), # Stdev ~= 406. Calculated from model 5's spectra csv.
    ...
    model5.compile(loss = 'mean_squared_error', optimizer = 'adam', metrics = ['mean_squared_error', 'mean_absolute_error'])
    model5.fit(TrainFeatures,TrainTargets,epochs = 500, batch_size = 4)
...
# RF, RT, VF, VT = model5_prep(use_wav = False)
# model5_train(save_to_disk = True, RF, RT, VF, VT)