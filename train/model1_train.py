...
def model1_prep(use_wav: bool):
    ...
    ValidationTargets = pd.read_csv('ValRegisters1.csv', index_col = 0)
    ValidationFeatures = pd.read_csv('ValSpectra1.csv', index_col = 0)
    
    if use_wav:
        SampleWav = pd.read_csv('SampleWav1.csv', index_col = 0)
        SampleFeatures = pd.concat([SampleFeatures, SampleWav], axis = 1, join = 'inner')
        del SampleWav
        ValWav = pd.read_csv('ValWav1.csv', index_col = 0)
...
def model1_train(save_to_disk: bool, TrainFeatures, TrainTargets, ValFeatures, ValTargets):
    # Building the model
    model1 = Sequential([
        layers.Input(shape = (81141,)),
        layers.Reshape((129, 629, 1)),
        layers.Resizing(height = 129, width = 129),
        layers.Normalization(mean = 126, variance = 165000), # Stdev ~= 406. Calculated from model 1's spectra csv.
    ...
    model1.compile(loss = 'mean_squared_error', optimizer = 'adam', metrics = ['mean_squared_error', 'mean_absolute_error'])
    model1.fit(TrainFeatures,TrainTargets,epochs = 400, batch_size = 3)
    if save_to_disk:
    ...
    loss  = model1.evaluate(ValFeatures, ValTargets)
...
# RF, RT, VF, VT = model1_prep(use_wav = False)
# model1_train(save_to_disk = True, RF, RT, VF, VT)