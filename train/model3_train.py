...
def model3_prep(use_wav: bool):
    ...
    ValidationTargets = pd.read_csv('ValRegisters3.csv', index_col = 0)
    ValidationFeatures = pd.read_csv('ValSpectra3.csv', index_col = 0)
    
    if use_wav:
        SampleWav = pd.read_csv('SampleWav3.csv', index_col = 0)
        SampleFeatures = pd.concat([SampleFeatures, SampleWav], axis = 1, join = 'inner')
        del SampleWav
        ValWav = pd.read_csv('ValWav3.csv', index_col = 0)
...
def model3_train(save_to_disk: bool, TrainFeatures, TrainTargets, ValFeatures, ValTargets):
    # Building the model
    model3 = Sequential([
        layers.Input(shape = (81141,)),
        layers.Reshape((129, 629, 1)),
        layers.Resizing(height = 129, width = 129),
        layers.Normalization(mean = 126, variance = 165000), # Stdev ~= 406. Calculated from model 3's spectra csv.
...
# RF, RT, VF, VT = model3_prep(use_wav = False)
# model3_train(save_to_disk = True, RF, RT, VF, VT)